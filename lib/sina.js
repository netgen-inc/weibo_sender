...
*/
var appKey = '4054929016';
var appSecret = 'd71d334237989dc8e544532108a09aae';
...
var Weibo = function(){
    ...
    this.send = function(blog){
        getAccount(blog.stock_code, function(account){
            send(account, blog);
        });
    };
    
    this.remove = function(sentBlog){
        getAccount(sentBlog.stock_code, function(account){
            remove(account, sentBlog);
        });
    };
    
    var send = function(account, blog){
        ...
        var req = http.request(options, function(res){
            ...
            res.on('end', function(){
                _self.emit('send', blog, res.statusCode, JSON.parse(body));
                console.log(body);
            });
        });
        req.write(paramStr);
    ...
    var remove = function(account, sentBlog){
        ...
        var req = http.request(options, function(res){
            ...
            res.on('end', function(){
                console.log(body);
                _self.emit('delete', sentBlog, res.statusCode, JSON.parse(body));
            });
        });
        req.write(paramStr);
        req.end();
    ...
    var getReqToken =  function(account){
        ...
        var hmac = crypto.createHmac('sha1', 'd71d334237989dc8e544532108a09aae&');
        hmac.update(signRaw);
        var sign = hmac.digest('base64');
        params.oauth_signature = sign;
