...
var Db = function(){
    ...
    _self.insertFollowerCount = function(){

    };

    _self.insertFollowerStatus = function(info){
        var statusDate = tool.getDateString();
        var sql = "INSERT INTO follower_status(follower_id,followers_count,friends_count,statuses_count,favourites_count,status_date)";
        sql += " VALUES(?, ? ,? ,? ,? ,?) ON DUPLICATE KEY UPDATE status_date = status_date";
        var data = [info.follower_id, info.followers_count, info.friends_count, info.statuses_count, info.favourites_count, statusDate];
