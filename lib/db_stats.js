...
var Db = function(){
    ...
    _self.insertMention = function(mention, callback){
        var sql = "INSERT INTO account_mention(" + 
            ...
            "ref_weibo_id, ref_weibo_content, ref_weibo_user_id, ref_weibo_sendtime, ref_weibo_url)" + 
            " VALUES(?,?,?,?,?,?,?,?,?,?,?,?, ?)"
        var mentionTime = tool.getDateString(new Date(mention.created_at));
        var refWeiboUrl = '', refWeiboId = '', refWeiboUserId = '', refWeiboContent = '', refWeiboSendTime = '0000-00-00';
        if(mention.retweeted_status && mention.retweeted_status.created_at){
            ...
            refWeiboSendTime = tool.getDateString(new Date(mention.retweeted_status.created_at));
            refWeiboUrl = mention.retweeted_status.t_url;
        }
        var data = [
            mention.account_id,
            mention.id.toString(),
